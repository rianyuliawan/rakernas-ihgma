          ...
          <label className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-2xl cursor-pointer active:scale-90 transition-all">
            <input type="file" hidden onChange={(e) => {
              ...
              if (file) {
                ...
                stopCamera().then(() => {
                  const currentS = new URLSearchParams(window.location.search).get("s") || "registrasi_ulang";
                  scannerRef.current?.scanFile(file, true).then(id => handleScanData(id, currentS));
                });
              }
            }} />
